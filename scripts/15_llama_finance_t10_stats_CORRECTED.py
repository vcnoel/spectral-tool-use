#!/usr/bin/env python3
"""
CORRECTED comprehensive statistics for Llama 3.1 8B Finance T=1.0.
Uses the SAME logistic regression method as the multi-detector sweep.
"""
import pandas as pd
import numpy as np
...
def analyze_config(name, features, df):
    ...
    if len(features) == 1:
        ...
        # CORRECTED: Use same logistic regression as multi_detector_sweep
        clf = LogisticRegression(max_iter=1000, random_state=42)  # NO class_weight='balanced'
        clf.fit(X, y)
...
print("="*80)
print("LLAMA 3.1 8B - FINANCE DATASET T=1.0 (N=1000) - CORRECTED")
print("="*80)
print("Using SAME logistic regression as multi-detector sweep (no class_weight)")
print()
...
for n_feat in [1, 2, 3, 4, 5]:
    ...
    if len(subset) > 0:
        ...
        if result:
            ...
            print(f"Best {n_feat}-feature: {' + '.join(result['feature_names'])}")
            print(f"  Reported AUC from sweep: {reported_auc:.4f}")
            print(f"  Recalculated AUC: {result['auc']:.4f} [{result['ci_lower']:.4f}, {result['ci_upper']:.4f}]")
            print(f"  Match: {'✓' if abs(result['auc'] - reported_auc) < 0.001 else '✗ MISMATCH!'}")
...
results_df.to_csv("data/llama_finance_1000_t10_comprehensive_stats_CORRECTED.csv", index=False)

print(f"{'='*80}")
print("CORRECTED RESULTS SAVED")
print(f"{'='*80}")
print("  data/llama_finance_1000_t10_comprehensive_stats_CORRECTED.csv")
